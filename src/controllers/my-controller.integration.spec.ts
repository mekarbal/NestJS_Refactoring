	...
	function createProducts(): ProductInsert[] {
		...
		return [
			...
			},
			{
				...
				{
				leadTime: 15, available: 10, type: 'FLASHSALE', name: 'Flash Sale Product', flashSaleEndDate: new Date(Date.now() - (2 * d)), flash_sale_end_date: new Date(Date.now() + (58 * d)),
			},
			{
				leadTime: 15, available: 20, type: 'FLASHSALE', name: 'Flash Sale Product2', flashSaleEndDate: new Date(Date.now() + (180 * d)), flash_sale_end_date: new Date(Date.now() + (240 * d)),
			},
		];
	}
});
